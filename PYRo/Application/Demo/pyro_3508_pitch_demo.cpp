 ...
 * @LastEditTime: 2025-11-12 22:38:02
...
extern "C"
{
    ...
    {
        vTaskDelay(500);
        pos_pid = new pyro::pid_ctrl_t(20.0f, 0.0f, 0.0f);
        pos_pid->set_output_limits(10.0f);
        pos_pid->set_integral_limits(1.0f);
        spd_pid = new pyro::pid_ctrl_t(50.0f, 0.0f, 0.001f);
        spd_pid->set_output_limits(20.0f);
        spd_pid->set_integral_limits(2.0f);
        ...
        pyro::rc_hub_t::get_instance(
            pyro::rc_hub_t::DR16)->config_rc_cmd([](void const *rc_ctrl)->void
            {
                static auto *p_ctrl = static_cast<pyro::dr16_drv_t::dr16_ctrl_t const*>(rc_ctrl);
                if (p_ctrl->rc.s[pyro::dr16_drv_t::DR16_SW_RIGHT].state != pyro::dr16_drv_t::DR16_SW_MID)
                {
                    zero_flag = true;
                }
                else if (zero_flag)
                ...
                if (!zero_flag)
                {
                    pitch_drv->update_target_radian(p_ctrl->rc.ch[pyro::dr16_drv_t::DR16_CH_LEFT_Y] * 0.005f);
                }
            });
        while (true)
        {
            ...
            {
                pitch_drv->calculate_torque(0.001f);
            }
